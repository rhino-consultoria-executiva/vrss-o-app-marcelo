...
export const getSupabaseConfig = () => {
    if (typeof window !== 'undefined') {
          ...
          if (localUrl && !localUrl.includes('jguexlunvzaiokcdelvy')) {
    ...
    const key = localKey || (import.meta as any).env.VITE_SUPABASE_ANON_KEY ||
                    ...
                    (import.meta as any).env.NEXT_PUBLIC_SUPABASE_ANON_KEY || 'sb_publishable_dsrkM1Mrkq6Nih_iU7y-Ig_rGSiNXo0';
...
export const mapLeadFromDb = (db: any): Lead => ({
    ...
    dateCreated: db.date_created || db.created_at || new Date().toISOString(),
...
export const mapServiceOrderFromDb = (db: any): ServiceOrder => ({
    ...
    notes: db.notes || '',
    dateCreated: db.date_created || db.created_at || new Date().toISOString().split('T')[0],
});
